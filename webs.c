...
int websOpenListen(int port, int retries)
{
	 ...
	 */
	for (i = 0; i<=retries; i++) {
		websListenSock = socketOpenConnection(
		                NULL,
		                port,
		                websAccept,
		                0);
...
int websAccept(int sid, char *ipaddr, int port, int listenSid)
{
	...
	ascToUni(
	                wp->ipaddr,
	                ipaddr,
	                min(sizeof(wp->ipaddr), strlen(ipaddr) + 1));
		...
		case WEBS_POST_CLEN:
			...
			if (wp->query) {
				if (wp->query[0]&&!(wp->flags&WEBS_POST_DATA)) {
					...
					if (text) {
						size = (len+gstrlen(text)+2)
						                *sizeof(char_t);
						wp->query =
						                brealloc(B_L, wp->query,
								                size);
						wp->query[len++ ] = '&';
#if !defined(WIN32)
					...
					{
						len = gstrlen(wp->query);
						size = (len+gstrlen(text)+1)
						                *sizeof(char_t);
						wp->query =
						                brealloc(B_L, wp->query, size);
						if (wp->query) {
#if !defined(WIN32)
							gstrcpy(
							                &wp->query[len],
...
static int websGetInput(webs_t wp, char_t **ptext, int *pnbytes)
{
	...
	*pnbytes = 0;
	...
	if (wp->state==WEBS_POST_CLEN) {
		len = (wp->clen>WEBS_SOCKET_BUFSIZ) ?
...
static void websParseRequest(webs_t wp)
{
	char_t *authType, *upperKey, *cp, *browser __attribute__ ((unused)),
	                *lp, *key, *value;
	...
	if (!(wp->flags&WEBS_HEADER_DONE)) {
		...
		if (code==401) {
			if (!(wp->flags&WEBS_AUTH_DIGEST)) {
				websWrite(
				                wp,
				                T("WWW-Authenticate: Basic realm=\"%s\"\r\n"),
				                websGetRealm());
#ifdef DIGEST_ACCESS_SUPPORT
			} else {
				char_t *nonce, *opaque;
...
void websRedirect(webs_t wp, char_t *url)
{
	...
	if (gstrstr(url, T("http://"))==NULL) {
		...
		fmtAlloc(
		                ...
		                WEBS_MAX_URL+80,
		                redirectFmt,
		                websGetVar(wp, T("HTTP_HOST"), websHostUrl),
		                url);
		url = urlbuf;
	}
...
void websError(webs_t wp, int code, char_t *fmt, ...)
{
	...
	msg =
	                T("<html><head><title>Document Error: %s</title></head>\r\n\
...
int websWrite(webs_t wp, char_t *fmt, ...)
{
	...
	if (fmtValloc(&buf, WEBS_BUFSIZE, fmt, vargs)>=WEBS_BUFSIZE) {
		trace(
		                0,
		                T("webs: websWrite lost data, buffer overflow\n"));
	}
